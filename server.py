"""
Nail POS — Local Server (Python 3)
Run: python server.py
"""
import json, os, socket, sys, sqlite3, threading
...
PORT      = 3000
_display  = {'state': 'idle', 'data': {}, 'tip': None}  # customer display (mutate, never reassign)
BASE_DIR  = os.path.dirname(os.path.abspath(__file__))
DATA_FILE = os.path.join(BASE_DIR, 'data.json')
GC_DB     = os.path.join(BASE_DIR, 'giftcards.db')

def gc_conn():
    """Return a sqlite3 connection to the gift card DB, or None if not found."""
...
# Auto-create giftcards.db schema if missing
def init_gc_db():
    import sqlite3 as _sq
    conn = _sq.connect(GC_DB)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # GET /api/giftcard?code=XX-XXXX-XXXX
        if path == '/api/giftcard':
            code = qs.get('code', [''])[0].strip()
            conn = gc_conn()
            if not conn:
                self.send_json(503, {'error': 'Gift card database not found'})
                return
            cur = conn.cursor()
            cur.execute(
                "SELECT gc_code,msr_id,cust_mobile,balance,status FROM gift_cards WHERE gc_code=? OR msr_id=?",
                (code, code)
            )
            row = cur.fetchone()
            conn.close()
        ...
        # GET /api/giftcards  — all cards for management tab
        if path == '/api/giftcards':
            ...
            if not conn:
                ...
                return
            cur = conn.cursor()
            cur.execute("""SELECT gc_code,msr_id,cust_mobile,balance,status FROM gift_cards
                           ORDER BY CASE status WHEN 'ACTIVE' THEN 0 WHEN 'PARTIALUSED' THEN 1 ELSE 2 END, gc_code""")
            rows = [dict(r) for r in cur.fetchall()]
            conn.close()
            self.send_json(200, rows)
            return

        # GET /api/giftcard/transactions?code=XX-XXXX-XXXX (optional filter)
        if path == '/api/giftcard/transactions':
            code = qs.get('code', [''])[0].strip()
            conn = gc_conn()
            if not conn:
                self.send_json(503, {'error': 'Gift card database not found'})
            ...
            cur = conn.cursor()
            if code:
                cur.execute("""SELECT gc_code,type,amount,balance_before,balance_after,ticket_no,note,created_at
                               FROM transactions WHERE gc_code=? ORDER BY created_at DESC, id DESC""", (code,))
            else:
                cur.execute("""SELECT gc_code,type,amount,balance_before,balance_after,ticket_no,note,created_at
                               FROM transactions ORDER BY created_at DESC, id DESC LIMIT 500""")
            rows = [dict(r) for r in cur.fetchall()]
            conn.close()
            self.send_json(200, rows)
            return
        ...
        if path == '/api/db':
            try:
                with _data_lock, open(DATA_FILE, 'rb') as f:
                    body = f.read()
            except:
                body = b'{}'
        ...
        if path == '/api/services':
            data = read_data()
            try:
                services = json.loads(data.get('npos_services', '[]'))
            except:
                services = []
        ...
        if path == '/api/settings':
            data = read_data()
            try:
                settings = json.loads(data.get('npos_settings', '{}'))
            except:
                settings = {}
            self.send_json(200, {'salonName': settings.get('salonName', 'Nail Salon')})
            return
        ...
        if path == '/api/bookings':
            data = read_data()
            try:
                pending = json.loads(data.get('npos_web_bookings', '[]'))
            except:
                pending = []
            self.send_json(200, pending)
            return

        # GET /api/display — customer display polling
        if path == '/api/display':
            self.send_json(200, _display)
            return
    ...
    def do_POST(self):
        ...
        # POST /api/bookings  — public booking page submits a pending request
        if path == '/api/bookings':
            try:
                ...
                try:
                    ...
                    pending = []
                booking['id'] = int(datetime.datetime.now().timestamp() * 1000) + random.randint(0, 999)
                booking['receivedAt'] = datetime.datetime.now().isoformat()
        ...
        # POST /api/giftcard/new  {gc_code, msr_id?, cust_mobile?, balance}
        if path == '/api/giftcard/new':
            try:
                ...
                if cur.fetchone():
                    conn.close(); self.send_json(400, {'error': f'Card code {gc_code} already exists'}); return
                if msr_id:
                ...
                cur.execute("""INSERT INTO gift_cards(gc_code,msr_id,cust_mobile,balance,status,created_at)
                               VALUES(?,?,?,?,?,?)""", (gc_code, msr_id or None, mobile or None, balance, status, now))
                cur.execute("""INSERT INTO transactions(gc_code,type,amount,balance_before,balance_after,ticket_no,note,created_at)
                               VALUES(?,?,?,?,?,?,?,?)""", (gc_code,'NEW',balance,0,balance,'','New card added via POS',now))
        ...
        # POST /api/giftcard/charge  {code, amount, ticket_no?}
        if path == '/api/giftcard/charge':
            try:
                ...
                row = cur.fetchone()
                if not row:
                    conn.close(); self.send_json(404, {'error': 'Card not found'}); return
                if row['status'] == 'USED':
                    conn.close(); self.send_json(400, {'error': 'Card is fully used'}); return
                bal_before = row['balance']
                if amount > bal_before:
                    conn.close(); self.send_json(400, {'error': f'Insufficient balance (${bal_before:.2f})'}); return
                ...
                conn.commit(); conn.close()
                self.send_json(200, {'ok': True, 'gc_code': row['gc_code'], 'balance_before': bal_before,
                                     'balance_after': bal_after, 'charged': amount})
            except Exception as e:
                self.send_json(400, {'error': str(e)})
            return

        # POST /api/giftcard/refill  {code, amount}
        if path == '/api/giftcard/refill':
            try:
                ...
                amount = float(body.get('amount', 0))
                if amount <= 0:
                    self.send_json(400, {'error': 'Amount must be positive'}); return
                conn = gc_conn()
                if not conn:
                    self.send_json(503, {'error': 'Gift card database not found'}); return
                cur = conn.cursor()
                ...
                conn.commit(); conn.close()
                self.send_json(200, {'ok': True, 'gc_code': row['gc_code'], 'balance_before': bal_before,
                                     'balance_after': bal_after, 'topped': amount})
            except Exception as e:
                self.send_json(400, {'error': str(e)})
            return

        # POST /api/db  — full DB snapshot from browser. Merged (not replaced) so that
        # server-owned keys the browser never tracks (e.g. npos_web_bookings, managed
        # via /api/bookings) survive a save instead of being wiped out.
        if path == '/api/db':
            try:
                body = self.read_body()
        ...
        # POST /api/display  — POS pushes order state to customer display
        if path == '/api/display':
            try:
                body = json.loads(self.read_body())
                _display.clear()
                _display.update(body)
                self.send_json(200, {'ok': True})
            except Exception as e:
                self.send_json(400, {'error': str(e)})
            return

        # POST /api/display/tip  — customer picks tip on display tablet
        if path == '/api/display/tip':
            try:
                body = json.loads(self.read_body())
                _display['tip'] = float(body.get('tip', 0))
                self.send_json(200, {'ok': True})
            except Exception as e:
                self.send_json(400, {'error': str(e)})
            return
        ...
        if path == '/api/terminal/sale':
            try:
                ...
                ip     = body.get('ip', '').strip()
                port   = int(body.get('port', 10009))
                amount = int(round(float(body.get('amount', 0)) * 100))   # cents
                tip    = int(round(float(body.get('tip', 0)) * 100))      # cents
    ...
    def do_DELETE(self):
        ...
        # DELETE /api/bookings/<id>  — staff dismiss/import a pending web booking
        if path.startswith('/api/bookings/'):
            try:
                booking_id = int(path.replace('/api/bookings/', ''))
                data = read_data()
                try:
                    pending = json.loads(data.get('npos_web_bookings', '[]'))
                except:
                    pending = []
                pending = [b for b in pending if b.get('id') != booking_id]
                data['npos_web_bookings'] = json.dumps(pending)
                write_data(data)
                self.send_json(200, {'ok': True})
            except Exception as e:
...
def _parse_response(data: bytes) -> dict:
    ...
    # T00 response fields (0-indexed after command):
    # 0=cmd, 1=version, 2=response_code, 3=response_msg,
    # 4=auth_code, 5=ref_num, 6=trans_id, 7=host, ...
    if len(parts) < 4:
...
def poslink_sale(ip: str, port: int, amount_cents: int, tip_cents: int, ref: str) -> dict:
    ...
    # POSLINK Credit Sale fields:
    # version, trans_type(01=sale), amount, tip, cashback, merch_fee, ecr_ref, ext_data
...
def poslink_void(ip: str, port: int, ref: str) -> dict:
    ...
    # void uses original ECRRefNum to find the transaction
...
if __name__ == '__main__':
    ...
    print(f'''
  ╔══════════════════════════════════════════╗
  ║          Nail POS Server v2.0            ║
  ╠══════════════════════════════════════════╣
  ║  Local:    http://localhost:{PORT}          ║
  ║  Network:  http://{local_ip}:{PORT}{'  ' if len(local_ip)<13 else ''}  ║
  ╚══════════════════════════════════════════╝
